...
class OfertaCotizacion
{
  ...
  SELECT
  of.id_oferta_cotizacion AS id,
  of.id_proveedor_sucursal AS id_proveedor_sucursal,
  of.id_cotizacion AS id_cotizacion,
  DATE_FORMAT(of.fecha_creacion, '%Y-%m-%d %H:%i:%s') AS fecha_creacion,
  DATE_FORMAT(of.fecha_modificacion, '%Y-%m-%d %H:%i:%s') AS fecha_modificacion,
  of.comentarios AS comentarios
  FROM oferta_cotizacion of";
    ...
    public static function getById($p_db, $p_id) {
        return self::getByParameter($p_db, "id_oferta_cotizacion", $p_id);
    }
    ...
    public static function seek($p_db, $p_parameters, $p_order, $p_direction, $p_offset, $p_limit, $p_get_total = false) {
        ...
        foreach($p_parameters as $key => $value) {
                            if ($key == 'id_oferta_cotizacion') {
                $array_clauses[] = "of.id_oferta_cotizacion = $value";
            }
                else             if ($key == 'id_cotizacion') {
                $array_clauses[] = "of.id_cotizacion = $value";
            }
                else             if ($key == 'id_proveedor_sucursal') {
                $array_clauses[] = "of.id_proveedor_sucursal = $value";
            }
                else         if (p.getKey().equals("mas reciente") {
            $array_clauses[] = "of.fecha_modificacion > STR_TO_DATE('" . $value . "', '%Y-%m-%d %H:%i:%s')";
        }
                else {
