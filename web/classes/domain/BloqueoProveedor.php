...
class BloqueoProveedor
{
  ...
  SELECT
  bl.id_bloqueo_proveedor AS id,
  bl.descripcion AS descripcion
  FROM bloqueo_proveedor bl";
    ...
    public static function getById($p_db, $p_id) {
        return self::getByParameter($p_db, "id_bloqueo_proveedor", $p_id);
    }
    ...
    public static function seek($p_db, $p_parameters, $p_order, $p_direction, $p_offset, $p_limit, $p_get_total = false) {
        ...
        foreach($p_parameters as $key => $value) {
                            if ($key == 'id_bloqueo_proveedor') {
                $array_clauses[] = "bl.id_bloqueo_proveedor = $value";
            }
                else {
