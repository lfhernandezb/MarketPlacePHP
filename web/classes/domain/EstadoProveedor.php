...
class EstadoProveedor
{
  ...
  SELECT
  es.id_estado_proveedor AS id,
  es.descripcion AS descripcion
  FROM estado_proveedor es";
    ...
    public static function getById($p_db, $p_id) {
        return self::getByParameter($p_db, "id_estado_proveedor", $p_id);
    }
    ...
    public static function seek($p_db, $p_parameters, $p_order, $p_direction, $p_offset, $p_limit, $p_get_total = false) {
        ...
        foreach($p_parameters as $key => $value) {
                            if ($key == 'id_estado_proveedor') {
                $array_clauses[] = "es.id_estado_proveedor = $value";
            }
                else {
